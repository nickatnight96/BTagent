"""Query plugin — SIEM/EDR query generation and execution assistance."""
...
class QueryPlugin(DefensivePlugin):
    """SIEM/EDR query generation and execution plugin.

    Provides tools for translating natural-language investigation questions
    into platform-specific queries (SPL, KQL, EQL, CrowdStrike) and
    executing them against connected data sources.
    """
    ...
    def get_tools(self) -> list[Any]:
        """Return LangChain tool instances for query operations."""
        return [query_generator, query_executor]

    def get_system_prompt(self) -> str:
        """Return the query agent system prompt.
