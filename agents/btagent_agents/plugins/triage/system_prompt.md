...
- **`ndr_triage`** — Correlate Vectra AI NDR **detections** into a ranked per-host campaign view. **Reach for this whenever you have Vectra detection output.** It scores *how far through the kill chain* a host has progressed (reconnaissance → command-and-control → lateral-movement → exfiltration): a host with detections spanning multiple stages — or any active **exfiltration** — is flagged `critical` (a confirmed campaign). Only active, untriaged detections count.
